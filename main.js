...
function createInitWindow(url) {
	...
	if (config.get('utilities_RPC') && checkURL(url) != 'social' && checkURL(url) != 'unknown') {
		DiscordRPC();
	}
	...
	shortcut.register(initWin, 'F2', () => {
		switch (checkURL(url)){
			case 'social': return;
			default: LinkBox(); return;
		}
	})
	
	shortcut.register(initWin, 'Alt+F4', () => {
		switch(checkURL(url)){
			case 'social': initWin.close(); break;
	...
	initWin.webContents.on('new-window', (event, url) => {
		...
		switch (e) {
			...
			case 'unknown':
				...
				break;
			case 'weird thing should die': app.quit(); return;
		}
	})
